...
import { CITIES, STATES, SKILLS } from "../lib/options";
...
export function Register() {
  const { role } = useParams(); // jobseeker | enterprise
  const toast = useToast();
  ...
  const submit = async (e) => {
    ...
    setBusy(true);
    try {
      const path = role === "enterprise" ? "/api/public/register/enterprise" : "/api/public/register/jobseeker";
      const body = role === "enterprise"
      ...
      setResult(res); toast(res.status);
    } catch (err) { toast(err.message, "error"); }
  ...
  return (
    <AuthShell wide
      title={isEnt ? "Register as an employer" : "Create your job seeker account"}
      subtitle={isEnt ? "Post jobs, search resumes and manage applications."
