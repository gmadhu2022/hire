...
const CATEGORIES = [
  { name: "Corporate", color: "text-navy" },
  { name: "Professional", color: "text-navy" },
  { name: "Skilled", color: "text-brandgreen-600" },
  { name: "Labour", color: "text-brandgreen-600" },
  { name: "Business", color: "text-navy" },
  { name: "Services", color: "text-brandgreen-600" },
];
    ...
    <div className="min-h-screen bg-white">
        ...
        <div className="relative mx-auto grid max-w-6xl items-center gap-10 px-6 py-16 lg:grid-cols-2">
          <div>
            ...
            <p className="mt-4 max-w-md text-lg text-slate-600">
              From daily-wage labour to post-graduate roles — one place for every job seeker,
            ...
            <div className="mt-8 flex flex-wrap gap-x-5 gap-y-2">
              {CATEGORIES.map((c) => (
                <span key={c.name} className={`text-sm font-semibold ${c.color}`}>{c.name}</span>
              ))}
            </div>
          </div>
          <div className="flex justify-center">
            <img src={logoFull} alt="Hire" className="w-full max-w-md drop-shadow-sm" />
          </div>
        </div>
      </section>
      ...
      <footer className="border-t border-slate-100 bg-white px-6 py-8">
        <div className="mx-auto flex max-w-6xl flex-col items-center justify-between gap-3 sm:flex-row">
          <Logo className="h-8" />
          <p className="text-xs text-slate-400">© {new Date().getFullYear()} Hire — Advanced job searching platform.</p>
        </div>
      </footer>
    </div>
  );
