...
def add_institute(body: schemas.InstituteBase, db: Session = Depends(get_db)):
    ...
    return schemas.CredentialResult(email=body.email, user_id=body.email,
                                    password=password, status="Institute Registration is Successful")
...
def reset_institute_password(institute_id: int, db: Session = Depends(get_db)):
    ...
    db.commit()
    send_credentials_email(inst.email, inst.name, inst.email, password)
    return schemas.CredentialResult(email=inst.email, user_id=inst.email,
                                    password=password, status="Password reset; email sent")
...
def add_enterprise(body: schemas.EnterpriseBase, db: Session = Depends(get_db)):
    ...
    send_credentials_email(body.email, body.name, body.email, password)
    return schemas.CredentialResult(email=body.email, user_id=body.email,
                                    password=password, status="Enterprise Registration is Successful")
...
def add_jobseeker(body: schemas.JobSeekerBase, db: Session = Depends(get_db)):
    ...
    return schemas.CredentialResult(email=body.email, user_id=body.email,
                                    password=password, status=f"Added {name} successfully")
